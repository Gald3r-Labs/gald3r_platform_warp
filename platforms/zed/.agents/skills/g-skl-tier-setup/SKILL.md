...
The interactive flow can run either in-chat (the agent asks and the user answers in conversation) or via the optional Python script at `gald3r tier setup` (shipped inside this skill folder for Cursor only, per D015 canonical-scripts pattern). Both paths produce identical file outputs. Agents should prefer the in-chat flow when the user is present — the script is for unattended/CI usage.
